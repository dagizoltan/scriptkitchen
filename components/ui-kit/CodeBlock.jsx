...
function highlightTokens(code, language) {
      ...
      if (token.type) {
        ...
        while ((match = regex.exec(token.text)) !== null) {
          if (match.index > lastIndex) {
            newTokens.push({ text: token.text.slice(lastIndex, match.index), type: null });
          }
          if (groupMap) {
            for (let i = 1; i < match.length; i++) {
              if (groupMap[i]) newTokens.push({ text: match[i], type: groupMap[i] });
            }
          } else {
  ...
  if (language === "css") {
    ...
    splitTokens(/\b(function|return|const|let|var|if|else|for|while|import|from|export|default|class|new|extends|super|this|throw|catch|try|await|async|typeof|in|of)\b/g, "keyword"); // keywords
    ...
    // Tags: <tag ...>
    splitTokens(/(&lt;\/?)([a-zA-Z0-9-]+)(.*?)(\/?&gt;)/g, null, {1: "punctuation", 2: "tag", 3: null, 4: "punctuation"});
    // Attributes inside tag
    splitTokens(/([a-zA-Z-:]+)(=)("[^"]*"|'[^']*')/g, null, {1: "property", 2: "operator", 3: "string"});
